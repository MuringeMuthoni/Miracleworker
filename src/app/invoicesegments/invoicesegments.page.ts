...
@Component({
  selector: 'app-invoicesegments',
  templateUrl: './invoicesegments.page.html',
  styleUrls: ['./invoicesegments.page.scss'],
})
export class InvoicesegmentsPage implements OnInit {
  selected_date;
  daBuilding
  ...
  segmentprev = "confirmation";
  segmentchangemode = "btt";
  
  divschedule:boolean=true
  divpayment:boolean=false
  divsummary:boolean=false
  constructor( private datePicker: DatePicker,public datepipe: DatePipe,private wcf :WcfServiceService ,private router:Router) {
  ...
  scheduleChecked(){
    ...
    console.log('selected ' + this.sched)
    if ( this.sched == "select_date"){
      this.divSeldate=true;
  ...
  Goto_payment(){
    ...
    if (this.deli == "no_del" && this.wcf.pickstation == "none"){
     // this.segmentModel = "schedule"
      this.router.navigate(['/delivery-pickup']);
    }else if (this.deli == "out" && this.wcf.Delivery_latlng == ""){
     // this.segmentModel = "schedule"
      this.router.navigate(['/delivery-location']);
    }else{
      this.segmentModel = "payment"
      this.segmentprev = "payment"
      this.segmentchangemode = "ok"

      this.divschedule=false
      this.divpayment=true
      this.divsummary=false
      
    }
   
    console.log('segments: ' + this.segmentModel +  ' ' + this.segmentprev);
